...
def _categories_page(papers: List[Paper], ctx: dict) -> str:
    ...
    content = f"""
<h1>Categories</h1>
<p>Every paper belongs to several categories — the electrochemical
transformation it reports and the cell, mediator or methodology behind it.
Click one to browse.</p>
...
organic electrosynthesis papers — synthesis in which electricity itself is the
...
electrophotochemistry, asymmetric electrosynthesis, flow electrosynthesis and
scale-up, and the cell and electrode design behind them.</p>
<h2>How it works</h2>
<p>Every day an automated pipeline queries open scholarly metadata APIs,
removes duplicates, classifies each paper with an electrosynthesis relevance
