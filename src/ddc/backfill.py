"""Historical backfill — index the whole history of organic electrosynthesis.

Walks OpenAlex (which aggregates Crossref, PubMed, arXiv and more, with
abstracts for classification) using cursor pagination:

* **Topic sweep**: a set of broad organic-electrosynthesis search queries, one
  year at a time, oldest publications to newest.
