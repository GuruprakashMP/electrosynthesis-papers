...
*electrochemistry is used as the reaction condition to make organic
molecules* — Baran-style e-synthesis. Any synthetic methodology qualifies,
but neighbouring electrochemical fields that are not synthesis (batteries and
supercapacitors, fuel cells, water-splitting electrocatalysis, corrosion,
electroplating, electroanalytical sensing) are rejected.
...
# Minimum evidence that organic electrosynthesis is the primary subject: one
# unambiguous term ("electrosynthesis", "anodic oxidation", "paired
# electrolysis"...) suffices; weak circumstantial matches alone do not.
...
def _compile(vocab: Dict[str, object]) -> List[Tuple[re.Pattern, str]]:
    ...
    "electrosynthesis" matches "electrosynthesis"; "electrochemical
    decarboxylat" matches "…decarboxylation" / "…decarboxylative".
    """
    compiled = []
    for phrase in vocab:
...
def classify(record: RawRecord) -> Classification:
    ...
    if prim_pts < _MIN_PRIMARY_POINTS:
        return Classification(False, 0, reason="no electrosynthesis evidence")
    ...
    # paper reads as "electrosynthesis topic + supporting details".
    ...
    if "General Electrosynthesis" in categories and len(categories) > 1:
        categories.remove("General Electrosynthesis")
